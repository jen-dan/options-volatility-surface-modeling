...
fit_path = OUTPUTS_DIR / "svi_fit_all_expiries.csv"
...
for expiry in expiries:
    ...
    plt.title(f"SVI Fit vs Market IV - {pd.Timestamp(expiry).date()}")
    plt.grid(True)
    plt.legend()

    out = OUTPUTS_DIR / f"svi_fit_vs_market_{pd.Timestamp(expiry).date()}.png"
